...
När du konfigurerar [!UICONTROL HTTP] >[!UICONTROL Make a request] modul, [!DNL Adobe Workfront Fusion] visar fälten som listas nedan. En rubrik med fet stil i en modul visar ett obligatoriskt fält.
  ...
  <tr> 
   ...
   <td> <p>HTTP-brödtexten är de databyte som skickas i ett HTTP-transaktionsmeddelande omedelbart efter rubrikerna om något ska användas.</p> 
    <ul> 
     ...
     <li> <p><strong>[!UICONTROL Application/x-www-form-urlencoded]</strong> </p> <p>Den här brödtypen är [!UICONTROL POST] data använda <code>[!UICONTROL application/x-www-form-urlencoded]</code>.</p> <p>För <code>application/x-www-form-urlencoded</code>, innehåller HTTP-meddelandet som skickas till servern i stort sett en frågesträng. Tangenterna och värdena kodas i nyckelvärdepar avgränsade med <code>&amp;</code> och med <code>=</code> mellan nyckeln och värdet. </p> <p>Använd för binära data <code>[!UICONTROL multipart/form-data]</code> i stället.</p> 
...
>[!INFO]
>
>**Exempel:** I det här exemplet visas hur du konfigurerar modulen för att skicka en [!UICONTROL POST] begäran med JSON-nyttolast:
>
>![](assets/make-a-request-example-350x522.png)

>[!NOTE]
>
>För att vara säker på att [!UICONTROL JSON] är giltig kan du använda någon av de tillgängliga onlinetjänsterna, som [https://jsonlint.com/](https://jsonlint.com/). Du kan också använda [!UICONTROL JSON] >[!UICONTROL Create JSON module] skapa JSON dynamiskt och ta hand om allt nödvändigt flytande.
>
>Blanda JSON-bitar med uttryck och objekt direkt i [!UICONTROL Request content] -fältet rekommenderas inte eftersom det kan resultera i ogiltig JSON.
