...
title: 'Exempel på Adobe Workfront Fusion-scenario: Koppla e-post, Textparser och Google Sheets'
...
# [!DNL Adobe Workfront Fusion] scenarioexempel: Koppla e-post, [!UICONTROL Text Parser]och [!DNL Google Sheets]
...
Den här självstudiekursen kräver grundläggande kunskaper i reguljära uttryck. Läs mer om Regex på [https://regexone.com](https://regexone.com/).
...
1. Sök efter e-post och välj **[!UICONTROL Watch emails]** som utlösare.
   ...
   >[!NOTE]
   >
   >När du ansluter en [!DNL Google] konto med hjälp av Ee-postmodul kan du även använda en [!DNL Gmail] -modul.

1. Anslut antingen en [!DNL Google] konto eller någon annan IMAP-baserad e-postklient (som [!DNL Outlook]).
1. När du är ansluten väljer du en mapp vars inkommande e-postmeddelanden du vill bevaka, till exempel [!UICONTROL Inbox].
1. Under [!UICONTROL Criteria], välja **[!UICONTROL All email]** (eller begränsa det till att läsa eller läsa e-post).
...
1. Klicka **[!UICONTROL Show advanced settings]** längst ned.
...
1. Filtrera e-postmeddelanden efter [!UICONTROL Sender address], [!UICONTROL Subject] och [!UICONTROL Phrase].
   ...
   >[!NOTE]
   >
   >Vi ska lägga till en router som letar efter fraser i ett e-postmeddelande med [!UICONTROL Match Pattern] iterator och ett reguljärt uttryck (Regex) som sökmönster. Detta gör det även möjligt för oss att skapa ett flerfunktionsscenario.
...
1. Fortsätt till [Sök efter [!UICONTROL Flow Control] och lägga till en [!UICONTROL Router]](#search-for-flow-control-and-add-a-router)

## Sök efter [!UICONTROL Flow Control] och lägga till en [!UICONTROL Router]

1. Lägg till en router efter en modul för att dela eller duplicera data innan du skickar dem till nästa modul.

   Här har vi använt en [!UICONTROL Router] för att skicka brödtext för e-post till två separata tabeller i en [!DNL Google Sheet].

   ![](assets/search-for-flow-control-350x220.png)

## Använd [!UICONTROL Text Parser] Modul

1. Lägg till en [!UICONTROL Match Pattern] för att söka efter en fras i ett e-postmeddelande.

   Vi ska söka efter frasen[!UICONTROL text parser module]&quot; i alla inkommande e-postmeddelanden för att fånga brödtexten och avsändarens namn på de som matchar frasen.
   ...
   1. I [!UICONTROL Text] klickar du på attributet **Textinnehåll** i listan.
...
1. Lägg till ytterligare [!UICONTROL Match Pattern] som söker efter samma fras och en e-postadress.
   ...
   Detta är särskilt användbart om du har kundkonton hos flera användare. För att spara tid kan du klona [!UICONTROL Text Parser] som du just har skapat och länkat den till routern.
...
## Lägg till [!DNL Google Sheets] moduler

För [!DNL Sheets]måste vi först skapa ett kalkylblad med de rubriker som behövs.

1. Skapa ett kalkylblad med de kolumner som du vill hämta användardata från. (Du kan använda en befintlig fil också).

   Skapa till exempel en som heter&quot;E-postdata: Support Ticket&quot; med avsändarens namn, avsändarens e-postadress och e-postinnehåll som kolumner. Namnge kalkylbladet &quot;innehåller: texttolkningsmodul.&quot;

1. Lägg till [!UICONTROL Google Sheets] modul med **[!UICONTROL Add a row]** som åtgärden.

   ![](assets/add-a-row-350x174.png)

1. Koppla samman [!DNL Google] konto (om du inte redan gjort det). Välj den fil du skapade tidigare och välj sedan det kalkylblad i vilket du hämtar data.
...
1. Klona modulen som du just skapade och länka den till den andra [!UICONTROL Text Parser] -modul.

   1. Gå till ditt kalkylblad, duplicera det kalkylblad du skapade tidigare och ge det ett namn.

      Ge det till exempel namnet&quot;innehåller: textanalysmodul och e-post.&quot;
   ...
   1. Klicka på den klonade [!DNL Google Sheets] för att konfigurera inställningarna.
   1. Ändra kalkylbladet till det nya som du just skapade.
   1. Mappa utdata från [!UICONTROL Match Pattern] modul ($1) till den kolumn där du vill lagra e-postadressen (delad e-postadress).

      ![](assets/map-the-output.png)

      ![](assets/sender-name-350x411.png)

   1. Klicka **[!UICONTROL OK]**, spara scenariot och ta det för en testkörning.
...
* [Läs om e-postmatchning](https://regexone.com/problem/matching_emails) med Regex
