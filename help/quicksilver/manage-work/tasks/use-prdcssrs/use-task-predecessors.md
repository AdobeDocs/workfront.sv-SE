---
content-type: reference
product-area: projects
navigation-topic: tasks
title: Använd föregående aktiviteter
description: Föregående aktiviteter är uppgifter som andra aktiviteter (som kallas efterföljande aktiviteter) är beroende av innan de kan slutföras. Läs mer om föregångare i Adobe Workfront i följande artiklar.
author: Alina
feature: Work Management
recommendations: noDisplay, noCatalog
exl-id: 56048927-39cd-49a0-b00a-bf145bca90a1
source-git-commit: d5f4e83badd4d011816551f06b056ffe886d3b17
workflow-type: tm+mt
source-wordcount: '80'
ht-degree: 0%

---

# Använd föregående aktiviteter

Det här avsnittet innehåller följande artiklar:

* [Översikt över föregående aktiviteter](../../../manage-work/tasks/use-prdcssrs/predecessors-overview.md)
* [Skapa en föregående relation i uppgiftslistan](../../../manage-work/tasks/use-prdcssrs/create-predecessors-on-task-list.md)
* [Skapa en föregående relation med området Föregående användare](../../../manage-work/tasks/use-prdcssrs/create-predecessors-in-predecessors-area.md)
* [Skapa föregående relationer genom att kedja aktiviteter](../../../manage-work/tasks/use-prdcssrs/create-predecessors-by-chaining-tasks.md)
* [Skapa föregångare mellan projekt](../../../manage-work/tasks/use-prdcssrs/cross-project-predecessors.md)
* [Översikt över aktivitetsberoendetyper](../../../manage-work/tasks/use-prdcssrs/task-dependency-types.md)
* [Tvinga föregående](../../../manage-work/tasks/use-prdcssrs/enforced-predecessors.md)
* [Översikt över fördröjda typer](../../../manage-work/tasks/use-prdcssrs/lag-types.md)
* [Översikt över uppgiftsberoendeloop](../../../manage-work/tasks/use-prdcssrs/dependency-loop-overview.md)
