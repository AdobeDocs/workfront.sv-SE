...
1. **CPI för uppgift 3** = .38, beräknat enligt följande:\
...
1. **CPI****för aktivitet 1** = .14 beräknat enligt följande:\
   **CPI** **för aktivitet 1** = *IF* Faktisk arbetskostnad + IncurradFaktiskKostnad &lt;> 0 *SEDAN*
...
1. **EAC****för aktivitet 1** = $13,400.00\
   ...
   **CPI Labor** **för aktivitet 1** = .04*****EAC Labor ****för aktivitet 1**=* IF *CPI_Labor &lt;> 0* THEN *EAC Labor = Planerad arbetskostnad/CPI_Labor
...
1. **CPI** för aktivitet 2 = 0,17 beräknat enligt följande:\
   **CPI-aktivitet 2** = *IF* Faktisk arbetskostnad + IncurradFaktiskUtgiftskostnad &lt;> 0 *SEDAN* CPI = (TotalBudgetedCostWorkPerformed + IncurrisedPlannedExpenseCost) / (ActualLaborCost + IncurrusActualExpenseCost)
   * ELSE* CPI = CPI_Labor\
...
1. CPI för uppgift 3 = 0,31, beräknad enligt följande:\
   ...
   * ELSE* CPI = CPI_Labor\
     ...
     **CPI****Aktivitet 3** = .31 ****** EAC för aktivitet 3 **= $9,521.74 beräknat enligt följande:\
