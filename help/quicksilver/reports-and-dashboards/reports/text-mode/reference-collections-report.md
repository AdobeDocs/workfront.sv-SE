...
Mer information om API Explorer-tabellen finns i [API Explorer](../../../wf-api/general/api-explorer.md).
...
1. Klicka på länken som visas för att gå till objektet i din samling.\
   Då öppnas **fält** -fliken för objektet i din samling.
...
1. Klicka på **Huvud** meny ![](assets/main-menu-icon.png)och sedan klicka **Rapporter**.
1. Klicka **Ny rapport**.
1. Markera rapportens objekt.
1. Navigera bort från rapporten och använd [API Explorer](../../../wf-api/general/api-explorer.md)bestämmer du vilka samlingar som är tillgängliga för det objekt som du har valt för rapporten.

   Mer information om hur du väljer objekt i samlingen finns i avsnittet [Söka efter samlingsobjekt och deras fält i API-utforskaren](#find-collection-objects-and-their-fields-in-the-api-explorer) i den här artikeln.\
   Notera namnet på objektet för samlingen.

1. Använda [API Explorer](../../../wf-api/general/api-explorer.md)går du till listan med fält för objektet som du vill visa i samlingen.

   Mer information om hur du söker efter fälten för objektet i din samling finns i avsnittet [Söka efter samlingsobjekt och deras fält i API-utforskaren](#find-collection-objects-and-their-fields-in-the-api-explorer) i den här artikeln.

   Notera namnet på det fält som du vill visa i samlingen.

1. Navigera tillbaka till rapporten och i **Kolumner (vy)** flik, klicka **Lägg till kolumn**.
1. Klicka **Växla till textläge**.
1. För musen över dialogrutan och klicka **Klicka för att redigera text**.
1. Markera all text i **Textläge** och ta bort den. Klistra sedan in följande kod om du refererar till ett fält i samlingsobjektet:
...
1. Ersätt **Kolumnnamn** med namnet på kolumnen i `displayname` linje.
1. Ersätt **samlingsobjektsnamn** med namnet på samlingsobjektet i `listmethod` linje, så som den visas i [API Explorer](../../../wf-api/general/api-explorer.md).

1. Ersätt **samlingsobjektfält** med namnet på samlingsobjektets fält i `valuefield` linje, så som den visas i [API Explorer](../../../wf-api/general/api-explorer.md).
...
1. (Valfritt) Fortsätt redigera rapporten.
   ...
   Klicka **Spara + Stäng** för att spara rapporten.
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>Du kan använda olika värden för den här raden, men vi rekommenderar att <code style="font-weight: normal;">valueformat</code> för en samlingslista bör <strong>HTML.</strong></p>
   </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>The <code>type</code> för en lista är alltid <code>iterate</code>när du skapar en vy.</p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>Ersätt <strong>Kolumnnamn</strong> med namnet på den nya kolumnen.</p> </td> 
  </tr> 
  <tr> 
   <td><code>listmethod=nested(collection object name).list</code> </td> 
   <td> <p> Den här raden definierar den samling som du refererar till.</p> <p>Ersätt <strong>samlingsobjektsnamn</strong> med namnet på det objekt som du refererar till i samlingen, så som det visas i <a href="../../../wf-api/general/api-explorer.md" class="MCXref xref">API Explorer</a>. Det här värdet är vanligtvis samlingsobjektets plural-form.</p> </td> 
  </tr> 
  <tr> 
...
1. Navigera tillbaka till rapporten och i **Filter** flik, klicka **Växla till textläge**.

1. I **Ange filterregler för rapporten** -området, klistra in följande kod:
...
1. Ersätt **samlingsobjektsnamn** med namnet på samlingsobjektet så som det visas i [API Explorer](../../../wf-api/general/api-explorer.md). Det här värdet är vanligtvis samlingsobjektets plural-form.

1. Ersätt **samlingsobjektfält** med namnet på samlingsobjektets fält i, så som det visas i [API Explorer](../../../wf-api/general/api-explorer.md).

1. Ersätt **värde för samlingsobjekt** med samlingsobjektets värde så som det visas i Workfront.
1. Ersätt **modifierarens värde** med en giltig modifierare.
...
1. Använd följande kod om du vill filtrera efter namnet på ett problem:
   ...
   >[!TIP]
   >
   >Observera att du måste använda `issues` för samlingsobjektets namn, i stället för `optask` vilket är hur problem visas i API-utforskaren.
...
Mer information om hur du skapar en filtersats med en samlingsreferens finns i avsnittet [Referera till en samling i ett rapportfilter](#reference-a-collection-in-the-filter-of-a-report) i den här artikeln.
...
1. Klicka **Ny rapport**.
1. Markera rapportens objekt.
1. Skapa ett filter med en samlingsreferens enligt beskrivningen i avsnittet [Referera till en samling i ett rapportfilter](#reference-a-collection-in-the-filter-of-a-report) i den här artikeln.
1. Klicka **Rapportinställningar**.
1. Klicka **Rapportera uppmaningar**.
1. Klicka **Lägg till fråga**.
1. Klicka **Anpassad fråga**.
1. Ange namnet på uppmaningen i dialogrutan **Fält****namn** fält.

1. Ange en **Etikett för listruteobjekt**.
1. Ange följande i dialogrutan **Villkor** fält:
...
1. Ersätt **samlingsobjektsnamn** med namnet på samlingsobjektet så som det visas i [API Explorer](../../../wf-api/general/api-explorer.md). Det här värdet är vanligtvis samlingsobjektets plural-form.
1. Ersätt **samlingsobjektfält** med namnet på samlingsobjektets fält så som det visas i [API Explorer](../../../wf-api/general/api-explorer.md).
1. Ersätt **värde för samlingsobjekt** med samlingsobjektets värde så som det visas i Workfront.

   Om du till exempel filtrerar efter projekt där namnet på uppgiften innehåller &quot;Marknadsföring&quot; ska du ersätta **värde för samlingsobjekt** med **marknadsföring**.

1. Ersätt **modifierarens värde** med en giltig modifierare.

   En lista med modifierare finns på  [Filter- och villkorsmodifierare](../../../reports-and-dashboards/reports/reporting-elements/filter-condition-modifiers.md).

   **Exempel:** Om du till exempel vill skapa en projektrapport med en anpassad fråga där du bara vill visa projekt som har minst en uppgift tilldelad en viss användare, använder du koden nedan:
   ...
   >[!NOTE]
   >
   >Du kan inte referera till namnet på den primära tilldelaren (&quot;Tilldelad till&quot;-fältet) för en uppgift enligt [API Explorer](../../../wf-api/general/api-explorer.md). Du kan bara referera till ID:t för den primära tilldelade personen.
