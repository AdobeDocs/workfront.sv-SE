...
1. Sök efter en posttyp eller välj något av följande:
   ...
   * En **ursprunglig begärandetyp** från avsnittet **Workfront-objekttyper**.
     ...
     Det ursprungliga anslutningsfältet för begäran som skapades visar namnet på den inskickade planeringsförfrågan som skapade objektet. Namnet på begäran visas också i fältet **Ämne** i begäran.

     >[!NOTE]
     >
     >Du måste skapa förfrågningsformulär och publicera dem för att kunna skicka in planeringsförfrågningar för att kunna skapa poster i Workfront Planning.
     >
   ...
   * **Experience Manager** i avsnittet **Adobe-program** om du vill ansluta till AEM Assets eller innehållsfragment.

     ![Val av AEM Assets-anslutning](assets/aem-assets-connection-selection-041326.png)

     En ny anslutningsflik öppnas för den valda posten eller objekttypen.

   * **Varumärken** från Adobe GenStudio for Performance Marketing i avsnittet **Adobe GenStudio**
...
1. (Villkorligt) När du väljer att ansluta till Experience Manager väljer du en databas i listrutan **Experience Manager-databas** i avsnittet **Länka resurser i följande databas**. Detta är ett obligatoriskt fält. Endast databaser som du har åtkomst till i Experience Manager visas i det här fältet.
...
1. Välj **Välj uppslagsfält** om du vill lägga till fält från den posttyp som du ansluter till. Uppslagsfälten är fält som är kopplade till den post eller objekttyp som du länkar till. När du länkar dem visas information från posten eller objektet som du länkar till på posten som du länkar från. Detta är markerat som standard.
   ...
   >[!TIP]
   >
   >* Förutom systemfält som Namn och Beskrivning kan du lägga till följande typer av fält som sökfält från objekttyper från ett annat program:
   >   * Användarfält, som Projektägare eller Sponsorn, från Workfront-projekt.
   >   * Referensfält, som projektgrupp eller projektföretag från Workfront-projekt.
